...
public class LoginController  {
    ...
    @Autowired
    RoleRepository roleRepositoryapi;
    @Autowired
    ...
    public String apiregistration(@RequestParam("user_name")String username,@RequestParam("user_email")String useremail, Model model) {

    /*   user Validator.validate(loginForm, result);
...
*/
        ...
        user.setPassword(bCryptPasswordEncoder.encode("imran123"));
        user.setRoles(new HashSet<>(roleRepositoryapi.findOneRole()));



        if(userRepository.findByUsername(user.getUsername())!=null)
        {
            securityService.autologin(user.getUsername(),"imran123");
            return "redirect:/welcome";
        }
        else
        {
